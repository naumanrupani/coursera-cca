...
public class TopNFinderBolt extends BaseBasicBolt {
 ...
 /*
    ...
    if ( currentTopWords.size() > this.N ) {
      String leastFrequencyWord = new String();
      Integer leastFrequency = Integer.MAX_VALUE;
      for(Map.Entry<String, Integer> item : currentTopWords.entrySet()) {
        Integer itemCount = item.getValue();
        if (itemCount < leastFrequency) {
          leastFrequency = itemCount;
          leastFrequencyWord = item.getKey();
        }
      }
      
      currentTopWords.remove(leastFrequencyWord);
    }
